...
function getJSONP(path, success, error)
{
...
function getGeoIPCountry() {
    ...
    var onSuccess = function(location){
        var successCountry = JSON.stringify(location["country"]["names"]["en"]);
      console.log(successCountry);
      updateQueryStringParam("utm_campaign","campaign_"+successCountry);
    };
...
function getLocation() {
    if (navigator.geolocation) {
        navigator.geolocation.watchPosition(function(position) {
            ...
            getJSONP("http://maps.googleapis.com/maps/api/geocode/json?latlng="+lat+","+lng+"&sensor=true",
             function(data) { var dataCountry = JSON.stringify(data["results"][10]["formatted_address"]); console.log(dataCountry); updateQueryStringParam("utm_campaign","campaign_"+dataCountry);},
             function(xhr) { console.error(JSON.stringify(xhr)); }
            );
        },
        function (error) { 
